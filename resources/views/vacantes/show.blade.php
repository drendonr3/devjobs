...
@section('content')
    ...
    <div class="mt-10 mb-20 md:flex items-start">
        <div class="md:w-3/5">
            ...
            <h2 class="text-2xl text-center mt-10 text-gray-700">Comocimientos y Tecnologías:</h2>
            @php
                $arraySkills = explode(",",$vacante->skills)
            @endphp
            @foreach($arraySkills as $skill)
                <p class="inline-block border border-gray-400 py-2 px-8 text-gray-700 my-2 rounded">
                    {{$skill}}
                </p>
            @endforeach

            <a href="/storage/vacantes/{{$vacante->imagen}}" 
                data-lightbox='imagen' data-title="Vacante {{$vacante->titulo}}">
                <img src="/storage/vacantes/{{$vacante->imagen}}" class="w-60 mt-10" alt="">
            </a>
